/*
------------------------------------------------------------------------------ 
This code was generated by Amplication. 
 
Changes to this file will be lost if the code is regenerated. 

There are other ways to to customize your code, see this doc to learn more
https://docs.amplication.com/how-to/custom-code

------------------------------------------------------------------------------
  */
import { ArgsType, Field } from "@nestjs/graphql";
import { ApiProperty } from "@nestjs/swagger";
import { VideoDetailsWhereInput } from "./VideoDetailsWhereInput";
import { IsOptional, ValidateNested, IsInt } from "class-validator";
import { Type } from "class-transformer";
import { VideoDetailsOrderByInput } from "./VideoDetailsOrderByInput";

@ArgsType()
class VideoDetailsFindManyArgs {
  @ApiProperty({
    required: false,
    type: () => VideoDetailsWhereInput,
  })
  @IsOptional()
  @ValidateNested()
  @Field(() => VideoDetailsWhereInput, { nullable: true })
  @Type(() => VideoDetailsWhereInput)
  where?: VideoDetailsWhereInput;

  @ApiProperty({
    required: false,
    type: [VideoDetailsOrderByInput],
  })
  @IsOptional()
  @ValidateNested({ each: true })
  @Field(() => [VideoDetailsOrderByInput], { nullable: true })
  @Type(() => VideoDetailsOrderByInput)
  orderBy?: Array<VideoDetailsOrderByInput>;

  @ApiProperty({
    required: false,
    type: Number,
  })
  @IsOptional()
  @IsInt()
  @Field(() => Number, { nullable: true })
  @Type(() => Number)
  skip?: number;

  @ApiProperty({
    required: false,
    type: Number,
  })
  @IsOptional()
  @IsInt()
  @Field(() => Number, { nullable: true })
  @Type(() => Number)
  take?: number;
}

export { VideoDetailsFindManyArgs as VideoDetailsFindManyArgs };
